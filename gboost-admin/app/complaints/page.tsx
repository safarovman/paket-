...
import { MOCK_COMPLAINTS, Complaint } from "@/lib/db";
...
export default function ComplaintsPage() {
  ...
  const handleStartReview = (id: string) => {
    setComplaints(p => p.map(c => c.id === id ? { ...c, status: "reviewing" } : c));
    showToast("🔍 Shikoyat ko'rib chiqilmoqda.");
  };

  const handleResolve = (id: string) => {
    if (!resolution.trim()) { showToast("Yechim/qaror yozing!", "error"); return; }
    setComplaints(p => p.map(c => c.id === id
      ? { ...c, status: "resolved", resolvedBy: "Admin", resolution }
      : c));
  ...
  const handleReject = (id: string) => {
    if (!resolution.trim()) { showToast("Rad etish sababini yozing!", "error"); return; }
    setComplaints(p => p.map(c => c.id === id
      ? { ...c, status: "rejected", resolvedBy: "Admin", resolution }
      : c));
    setSelected(null); setResolution("");
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-6">
        ...
        <p className="text-text-gray text-sm">Foydalanuvchilardan tushgan shikoyatlarni ko'rib chiqing</p>
      </div>
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          ...
          <div key={c.id} className="acard hover:border-border/80 transition-all">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  ...
                  <span className="text-text-gray text-xs">{c.id} • {c.createdAt}</span>
                </div>
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  <span className="text-white font-bold text-sm">{c.fromUser}</span>
                  <span className="text-text-gray text-xs">→ shikoyat →</span>
                  <span className="text-red font-semibold text-sm">{c.againstUser}</span>
                  <span className="text-text-gray text-xs">({c.orderId})</span>
                </div>
                <p className="text-text-gray text-sm leading-relaxed line-clamp-2">{c.description}</p>
                {c.resolution && (
                  <div className="mt-2 p-2.5 bg-green/10 border border-green/20 rounded-xl">
                    ...
                    <p className="text-text-gray text-xs">— {c.resolvedBy}</p>
                  </div>
                )}
              </div>
              <div className="flex gap-2 flex-wrap shrink-0">
                {c.status === "new" && (
                  <button onClick={() => handleStartReview(c.id)}
                    className="abtn-ghost border border-border text-xs px-3 py-1.5 rounded-xl">
                    🔍 Ko'rib chiqish
                  </button>
                )}
                {(c.status === "new" || c.status === "reviewing") && (
                  <button onClick={() => setSelected(c)}
                    className="text-xs bg-purple/20 border border-purple/30 text-purple hover:bg-purple/30 px-3 py-1.5 rounded-xl transition-all font-semibold">
                    Qaror berish
                  </button>
                )}
              </div>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg p-6">
            ...
            <div className="space-y-3 mb-5">
              <div className="bg-bg border border-border rounded-xl p-4">
                ...
                <p className="text-white text-sm font-semibold mb-1">
                  {selected.fromUser} → {selected.againstUser}
                </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={() => handleReject(selected.id)}
                className="bg-red/15 border border-red/30 text-red hover:bg-red/25 py-3 rounded-xl text-sm font-bold transition-all">
                ❌ Rad etish
              </button>
            </div>
          </div>
